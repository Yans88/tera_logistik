...
<div class="modal fade" role="dialog" id="frm_category">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body" style="padding-bottom:2px;">
				...
				<div class="form-group">
                  <label>Code</label><span class="label label-danger pull-right form_code_error"></span>
                  <input type="text" class="form-control" name="form_code" id="form_code" value="" placeholder="Code" autocomplete="off" />
                 
                </div>
				<div class="form-group">
                  <label>Title</label><span class="label label-danger pull-right region_error"></span>
                  <input type="text" class="form-control" name="form_title" id="form_title" value="" placeholder="Title" autocomplete="off" />
                  <input type="hidden" value="" name="id_form" id="id_form">
 ...
 <div class="box-header">
    <button class="btn btn-success add_category"><i class="fa fa-plus"></i> Add Form</button>
	<ol class="breadcrumb">
		...
		<li class="active"><a><?php echo $m_form->nama_form;?></a></li>
...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:56%">Form</th>			
		...
		<tbody>
			<?php 
				...
				if(!empty($master_form)){		
					foreach($master_form as $r){	
						$view_sub = '';
						$view_sub = site_url('list_form/list_input/'.$r['id']);
						$info = $r['id'].'Þ'.$r['title'].'Þ'.$r['code'];
						echo '<tr>';
			...
			<a href="'.$view_sub.'"><button title="List Inputan" class="btn btn-xs btn-warning"><i class="fa fa-list-ul"></i> List Inputan</button></a>
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
			<button title="Delete" id="'.$r['id'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('list_form/del_type');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	$('.region_error').text('');
	if(title <= 0 || title == '') {
		$('.region_error').text('Title harus diisi');
		return false;
	}	
	var dt = $('#frm_cat').serialize();
	var url = '<?php echo site_url('list_form/simpan_type');?>';
	$.ajax({
		data:dt,
		type:'POST',
		url : url,
		success:function(response){
			...
			if(response == 'taken'){
				$('.region_error').text('Title sudah digunakan');
				return false;
			}
			if(response > 0){
