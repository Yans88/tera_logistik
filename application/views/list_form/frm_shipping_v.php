...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:66%">Form</th>			
			...
			<?php 
				...
				if(!empty($master_form)){		
					foreach($master_form as $r){	
						$view_sub = '';
						$view_sub = site_url('list_form/m_frm/'.$r['id_shipping']);
						$info = $r['id_shipping'].'Þ'.$r['shipping_name'];
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';						
						echo '<td>'.$r['shipping_name'].'</td>';	
						
						echo '<td align="center" style="vertical-align: middle;">		
			<a href="'.$view_sub.'"><button title="List Form" class="btn btn-xs btn-warning"><i class="fa fa-list-ul"></i> List Form</button></a>
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
			<button title="Delete" id="'.$r['id_shipping'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('list_form/del_shipping');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	var url = '<?php echo site_url('list_form/simpan_shipping');?>';
	$.ajax({
		data:dt,
		type:'POST',
		url : url,
